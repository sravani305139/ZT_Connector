...
def to_xml(data,name_of_file):#creates the XML files
    try:
        #xml = dicttoxml(data, custom_root='test', attr_type=False)   s                                                                  #converts the dictonary data into XML
...
def json2xml(json_obj, Line_spacing=""):
   ...
   if json_obj_type is dict:
       for tag_name in json_obj:
           ...
           result_list.append("%s</%s>" % (Line_spacing, tag_name))
           # print(result_list)
       return "".join(result_list)      # <-- this one was misindented
...
def rest_data(link):
    try:
        ...
        # Do the HTTP request
        s = requests.Session()
        response = s.get(url, auth=("sravani", "wipro@123"), headers=headers )
        info("Fetching data from Instance "+str(Instance))
        if response.status_code != 200: 
            print('Status:', response.status_code, 'Headers:', response.headers, 'Error Response:',response.json())
            info('Status:', response.status_code, 'Headers:', response.headers, 'Error Response:',response.json())
            exit()
        # Decode the JSON response into a dictionary and use the data
        #print(response.content)
        SN_data = response.json()
        #print(SN_data)
        #print(len(SN_data['result']))
        i = 0
        for row in SN_data['result']:
            info(SN_data['result'][i])
            info("#############################################################")
            dic.append(SN_data['result'][i])
            i+=1
        print(dic)
        return dic
            #print(i)
...
def read_ticket(groupname,status):
    try:
        ...
        esc = 0
        found = 0
        name_field = "?sysparm_fields=name"
            ...
            if ((result['sys_updated_on']) > prvtime):                               # compares with previous time
                if((result['sys_updated_on']) <= curr_time) :                        # compares with the captured current time          
                    for key in result:
                        ...
                        print(type(val))                                                                               #reads the result for the provided attributes)
                        if type(val)==dict:
                            #fetch_name = snow_client.resource(api_path=val['link'])
                            print(rest_data(val['link']+name_field))
                    dic = json2xml(result)
                    name_of_file = result['number']
                    Xml_data = to_xml(dic,name_of_file)
                    '''
